...
**View screen:** `/salaries` (`Salaries.razor`) — read-only table of uploaded salary rows. Entry points: context button on Main Dashboard; hub card on Year Management. Period is a row of school-year month buttons (year `StartDate`–`EndDate`, typically Sept–Aug) with totals per month; default selected month is the previous calendar month when it falls in that year. From Year Management (`?fromYear=`) the year is fixed; from the main dashboard a Hebrew-year dropdown appears above the buttons. Summary dashboard above the remaining filters (computed from the loaded period): record count, total salary, distinct national IDs, unmatched-to-person rows (count + sum, clickable drill-down), matched rows without an active allocation overlapping the salary month (count + sum, clickable drill-down), ID warning count. Other filters: national ID, department, matched-to-person, allocation-for-period, ID warning. Rows linked to `salary_anomalies.salary_id` (latest process for the period) show a red bell; click opens a read-only anomaly modal. API: `GET api/salaries?year=&month=` — each row includes `HasAllocationForPeriod`, derived from the stored `matched_allocation_id`. Anomalies for the bell: `GET api/salary-anomalies?year=&month=`. Month button totals: `GET api/salaries/month-totals?fromYear=&fromMonth=&toYear=&toMonth=`. Security actions: `salaries_page_action`, `salaries_back`, `salaries_refresh`, `salaries_recheck`, `maindashboard_salaries_view`, `yearmanagement_salaries_view`. SQL: `add-salaries-view-actions.sql`, `add-salaries-recheck-action.sql`.
...
Payroll `department_id` values vary per authority. Tenant map `assist_schema.salary_department_mappings` (`department_id` → `assistant_type_id`, unique per entity). Inactive mappings are treated as unmapped. UI: `/salary-department-mappings`, opened from Main Dashboard and Year Management (not from the salaries table). SQL: `add-monthly-ops.sql`.
...
**UI (entitlements screen):** Main table columns (client-side sortable except Actions): פעולות, סוג, סוג מוסד (גן / יסודי / תיכון from institution type + school level), מוסד, סוג סייעת, שעות, השתתפות משרד, הקצאה (icon: red dot none / yellow check partial / green check full), תאריכים, סטטוס. Class name, pupil name, national ID, and class classification are not in the table — they appear on the details tab. Selecting a row opens a height-resizable bottom dock with two tabs: **פרטי זכאות** (default; same fields as the edit modal, plus ערוך) and **הקצאות**. History remains available via `GET api/entitlements/history/{masterEntitlementId}`.
...
**View screen:** `/meitar-data` (`MeitarData.razor`) — read-only tables of stored `meitar_mutavim`, `meitar_mucarim` and `meitar_sharatim` rows, switched via a MUTAVIM/MUCARIM/SHARATIM tab toggle (all three datasets are loaded together on each period change; switching tabs is client-side only). Entry points: context button on Main Dashboard (`/meitar-data`); hub card on Year Management (`/meitar-data?fromYear={yearId}`). Period filter is calendar year + month (server reload) applied to a date column, not to `period_year`/`period_month`, and is shared by all tabs:
