...
namespace PetelAssistants.Api.Controllers
{
    ...
    public class PersonalEntitlementUploadController : BaseController
    {
        private readonly PersonalEntitlementFileProcessor _processor;

        public PersonalEntitlementUploadController(
            PersonalEntitlementFileProcessor processor,
            UserSessionService sessionService,
            ILogger<PersonalEntitlementUploadController> logger)
        ...
        [HttpPut("mapping")]
        public async Task<IActionResult> SaveMapping([FromBody] PersonalEntitlementFieldMappingSaveRequest request)
        {
            var session = GetCurrentSession();
            if (session == null)
                return Unauthorized(new { success = false, message = "נדרש אימות" });
            ...
            var mappingError = PersonalEntitlementFileProcessor.ValidateMapping(mapping);
        ...
        public async Task<IActionResult> PreviewFile([FromForm] PersonalEntitlementFilePreviewRequest request)
        {
            var session = GetCurrentSession();
            if (session == null)
                return Unauthorized(new { success = false, message = "נדרש אימות" });
            ...
            try
            {
                ...
                var suggestedMappings = PersonalEntitlementFileProcessor.GenerateSuggestedMappings(headers);
                var saved = await _processor.GetSavedMappingAsync();
                ...
                return Ok(new
                {
                    ...
                    headers,
                    suggestedMappings,
                    availableFields = PersonalEntitlementFileProcessor.GetAvailableFields(),
            ...
            {
                _logger.LogError(ex, "Error previewing personal entitlement file");
        ...
        [Consumes("multipart/form-data")]
        [RequestSizeLimit(10_000_000)]
        public async Task<IActionResult> UploadFile([FromForm] PersonalEntitlementFileUploadRequest request)
        {
            var session = GetCurrentSession();
            if (session == null)
                return Unauthorized(new { success = false, message = "נדרש אימות" });
            ...
            {
                _logger.LogError(ex, "Error uploading personal entitlement file");
                return StatusCode(500, new { success = false, message = "שגיאה בעיבוד הקובץ: " + ex.Message });
            }
        }

        [HttpPost("cancel-orphans")]
        public async Task<IActionResult> CancelOrphans([FromBody] PersonalEntitlementCancelOrphansRequest request)
        {
            var session = GetCurrentSession();
            if (session == null)
                return Unauthorized(new { success = false, message = "נדרש אימות" });
            ...
            {
                _logger.LogError(ex, "Error cancelling orphan personal entitlements");
