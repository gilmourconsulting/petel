...
{
    [ApiController]
    [Route("api/meitar-month-summaries")]
    public class MeitarMonthSummariesController : BaseController
    {
        ...
        public async Task<IActionResult> Get([FromQuery] int year, [FromQuery] int month)
        {
            ...
            var processId = await _context.MeitarRetrieveProcesses
            ...
            var hasRows = await _context.MeitarMonthSummaries.AnyAsync(s => s.ProcessId == processId.Value);
            if (!hasRows)
                await _comparisonService.RebuildMeitarProcessAsync(processId.Value, userId);

            var lines = await _context.MeitarMonthSummaries
                .AsNoTracking()
