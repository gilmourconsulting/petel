-- =============================================================================
-- PetelAssistants — Yearly budget "add assistant types" button security action
...
BEGIN
    ...
    VALUES
        ('yearly_budget_add_types', 'הוספת סוגי סייעת לתקציב שנתי', 'yearly_budget', 'כפתור הוספת סוגי סייעת חסרים לגרסת תקציב פתוחה', v_button_type_id)
    ON CONFLICT (name) DO NOTHING;

    SELECT id INTO v_action_id
    FROM shared_schema.actions
    WHERE name = 'yearly_budget_add_types';
    ...
    RAISE NOTICE 'Yearly budget add-types security action seeded';
END $$;
