# ============================================
# PetelATH - Azure Deployment Script
# ============================================
# Deploys the ATH API and/or Blazor Server
...
$AthRootPath = Join-Path $RootPath "PetelATH"
$BlazorProjectPath = Join-Path $AthRootPath "PetelATH.BlazorServer"
$ApiProjectPath = Join-Path $AthRootPath "PetelATH.Api"

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "PetelATH - $Environment Deployment" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $ApiOnly) {
    Write-Step "Deploying ATH Blazor Server Application"
    ...
    Remove-Item "ath-blazor-deploy-$Environment.zip" -Force -ErrorAction SilentlyContinue
    Push-Location (Join-Path $BlazorProjectPath "publish")
    tar.exe -a -c -f (Join-Path $RootPath "ath-blazor-deploy-$Environment.zip") *
    Pop-Location
    
    $packageSize = (Get-Item "ath-blazor-deploy-$Environment.zip").Length / 1MB
    ...
    $deployResult = az webapp deploy `
        --resource-group $ResourceGroup `
        --name $BlazorAppName `
        --src-path "ath-blazor-deploy-$Environment.zip" `
        --type zip `
        --restart true `
        --timeout 300 `
...
# Deploy API
if (-not $BlazorOnly) {
    Write-Step "Deploying ATH API Application"
    ...
    Write-Host "Creating deployment package..." -ForegroundColor Gray
    Set-Location $RootPath
    Remove-Item "ath-api-deploy-$Environment.zip" -Force -ErrorAction SilentlyContinue
    Push-Location (Join-Path $ApiProjectPath "publish")
    tar.exe -a -c -f (Join-Path $RootPath "ath-api-deploy-$Environment.zip") *
    Pop-Location
    
    $packageSize = (Get-Item "ath-api-deploy-$Environment.zip").Length / 1MB
    Write-Host "Package size: $([math]::Round($packageSize, 2)) MB" -ForegroundColor Gray
    ...
    $deployResult = az webapp deploy `
        --resource-group $ResourceGroup `
        --name $ApiAppName `
        --src-path "ath-api-deploy-$Environment.zip" `
        --type zip `
        --restart true `
        --timeout 300 `
        --only-show-errors 2>&1
...
Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "ATH DEPLOYMENT COMPLETE!" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $ApiOnly) {
    Write-Host "  ath-blazor-deploy-$Environment.zip" -ForegroundColor Gray
}
if (-not $BlazorOnly) {
    Write-Host "  ath-api-deploy-$Environment.zip" -ForegroundColor Gray
}

Write-Host ""
Write-Host "Usage examples:" -ForegroundColor Cyan
Write-Host "  Deploy both:      .\Deploy-ATH.ps1 -Environment $Environment" -ForegroundColor Gray
Write-Host "  API only:         .\Deploy-ATH.ps1 -Environment $Environment -ApiOnly" -ForegroundColor Gray
Write-Host "  Blazor only:      .\Deploy-ATH.ps1 -Environment $Environment -BlazorOnly" -ForegroundColor Gray
Write-Host "  Skip build:       .\Deploy-ATH.ps1 -Environment $Environment -SkipBuild" -ForegroundColor Gray
Write-Host "  Skip IP config:   .\Deploy-ATH.ps1 -Environment $Environment -SkipIpRestrictions" -ForegroundColor Gray
Write-Host ""
